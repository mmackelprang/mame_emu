...
| 0005 | In progress | 5 | Tooling-only. **Boundary H shipped** (source-staleness detector + `make check-sources`, PR #2). **Boundary I shipped** (`reconcilelist --fix` autofix + byte-exact golden tests; CI runs check mode only). **Boundary J pending** (fast CI pre-flight + docs). Unit-tested. |
